...
const  disableButton = computed(() => props.isCreatingOrder.value ? true : props.totalPrice ? false : true);
</script>
...
<infoblock  title="Корзина пустая " description="Добавте товар  в корзину" imageUrl="/package-icon.png" />
    </div>
   <CartListitem v-if="totalPrice"/>
    ...
    <div v-if="totalPrice" class="flex flex-col gap-4 mt-7">
      ...
      :disabled="disableButton" class=" mt-4 flex justify-center cursor-pointer items-center gap-3 w-full py-3  disabled:bg-slate-300 bg-lime-500 text-white rounded-xl transition active:bg-lime-700 hover:bg-lime-600">
